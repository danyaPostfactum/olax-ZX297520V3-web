...
require("set service lib".split(" "), function(e, d, a) {
    function b(f) {
        require([f.menu, f.set], function(g) {
            require("menu language logout statusBar router entry jq_additional jq_translate jq_simplemodal base64".split(" "), function(l, m, h, j, i, k) {
                l.init();
                m.init();
                i.init();
                h.init();
    ...
    if (!e.RJ45_SUPPORT) {
        ...
        var c = "menu";
        d.getOpMode({}, function(f) {
            e.blc_wan_mode = f.blc_wan_mode;
            switch (f.blc_wan_mode) {
                case "AUTO_PPPOE":
                case "PPPOE":
                    c = "menu_pppoe";
                    break;
