...
define("ota_update", "jquery jq_fileinput service knockout set statusBar".split(" "), function(d, h, e, k, b, c) {
    function a() {
        var m = this;
        var l = e.getOTAUpdateSetting();
        m.allowRoamingUpdate = k.observable(l.allowRoamingUpdate);
        m.hasDdns = b.DDNS_SUPPORT;
        m.hasUpdateCheck = b.HAS_UPDATE_CHECK;
        m.hasUssd = b.HAS_USSD;
        m.isDataCard = b.PRODUCT_TYPE == "DATACARD";
        m.lastCheckTime = k.observable("");
        m.updateIntervalDay = k.observable(l.updateIntervalDay);
        m.updateMode = k.observable(l.updateMode);
        m.updateType = k.observable(e.getUpdateType().update_type);
        m.apply = function() {
            var n = {
                updateMode: m.updateMode(),
                updateIntervalDay: m.updateIntervalDay(),
                allowRoamingUpdate: m.allowRoamingUpdate()
            };
            showLoading();
            e.setOTAUpdateSetting(n, function(o) {
                if (o && o.result == "success") {
                    l.allowRoamingUpdate = m.allowRoamingUpdate();
                    successOverlay()
                } else {
                    errorOverlay()
                }
        ...
        m.clickAllowRoamingUpdate = function() {
            var n = d("#chkUpdateRoamPermission:checked");
            if (n && n.length == 0) {
                m.allowRoamingUpdate("1")
            } else {
                m.allowRoamingUpdate("0")
            }
        };
        e.getOTAlastCheckTime({}, function(n) {
            m.lastCheckTime(n.dm_last_check_time)
        })
    }

    function i(n) {
        var m = 0;
        var o = /msie/i.test(navigator.userAgent) && !window.opera;
        if (o) {
            var l = n.value;
            try {
                var p = new ActiveXObject("Scripting.FileSystemObject");
                m = parseInt(p.GetFile(l).size)
            } catch (q) {
                m = 1
            }
        } else {
            try {
                m = parseInt(n.files[0].size)
            } catch (q) {
                m = 1
            }
        }
    ...
    fileUploadSubmitClickHandler = function() {
        var n = d(".customfile").attr("title");
        if (typeof n == "undefined" || n == "" || n == d.i18n.prop("no_file_selected")) {
            showAlert("sd_no_file_selected");
            return false
        } else {
            var m = n.substring(n.lastIndexOf(".")).toLowerCase();
            var l = i(d("#fileField")[0]);
            if (l > b.NATIVE_UPDATE_FILE_SIZE) {
                showAlert("error_file_selected");
                return false
            }
        }
        showLoading("uploading", '<span data-trans="upload_tip">' + d.i18n.prop("upload_tip") + "</span>");
        if (!g) {
            f()
        }
        d("#fileUploadForm").submit()
    };
    var g = false;

    function f() {
        g = true;
        d("#fileUploadIframe").load(function() {
            var m = d("#fileUploadIframe").contents().find("body").html().toLowerCase();
            d("#fileField").closest(".customfile").before('<input id="fileField" name="filename" maxlength="200" type="file" dir="ltr"/>').remove();
            addTimeout(function() {
                d("#fileField").customFileInput()
            }, 0);
            var l = false;
            if (m.indexOf("success") != -1) {
                showAlert("upload_update_success")
            } else {
                if (m.indexOf("failure0") != -1) {
                    showAlert("upload_update_failed0")
                } else {
                    if (m.indexOf("failure1") != -1) {
                        showAlert("upload_update_failed1")
                    } else {
                        if (m.indexOf("failure2") != -1) {
                            showAlert("upload_update_failed2")
                        } else {
                            if (m.indexOf("failure3") != -1) {
                                showAlert("upload_update_failed3")
                            } else {
                                if (m.indexOf("failure4") != -1) {
            ...
            d("#uploadBtn", "#uploadSection").attr("data-trans", "browse_btn").html(d.i18n.prop("browse_btn"));
            d(".customfile", "#uploadSection").removeAttr("title");
            d(".customfile span.customfile-feedback", "#uploadSection").html('<span data-trans="no_file_selected">' + d.i18n.prop("no_file_selected") + "</span>").attr("class", "customfile-feedback")
        })
    }
...
define("sd_httpshare", "jquery underscore jq_fileinput set service knockout".split(" "), function(y, M, i, C, g, K) {
    ...
    function I(P) {
        ...
        y("#fileList_container").html(y.tmpl("sdFileItemTmpl", {
            data: N
        }))
    }
    ...
    pagination = function(N) {
        ...
        y(".pager", "#fileListButtonSection").html(y.tmpl("pagerTmpl", {
            data: {
                pagers: O,
                total: J(N, j)
            }
    ...
    function x() {
        y("#createNewFolderForm").validate({
            ...
            rules: {
                newFolderName: {
                    sd_card_path_too_long: true,
                    check_filefold_name: true
                }
            }
        });
...
define("sms_list", "underscore jquery knockout set service jq_chosen".split(" "), function(Y, B, T, G, d, K) {
            ...
            if (aa) {
                showConfirm(ab, {
                    ok: function() {
                        if (!aa) {
                            F({
                                content: B("#chat-input", "#smsChatRoom").val(),
                                numbers: Z,
                                isFromBack: true
                            })
                        }
    ...
    gotoBottom = function() {
        B("#chatpanel .clear-container").animate({
            scrollTop: B("#chatlist").height()
        })
    };
    var L = 0;
    var W = false;
...
define("sms_sim_messages", "jquery knockout set service".split(" "), function(e, b, o, s) {
    ...
    function h(t) {
        ...
        cleanSimSmsList();
        e("#simMsgList_container").html(e.tmpl("simMsgListTmpl", {
            data: t
        }));
        hideLoading()
    }
