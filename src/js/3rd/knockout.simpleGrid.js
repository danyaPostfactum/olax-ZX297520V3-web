(function() {
    ...
    ko.simpleGrid = {
        viewModel: function(d) {
            var c = this;
            c.sideNumber = 3;
            c.midNumber = 1 + 2 * c.sideNumber;
            c.allNumber = c.midNumber + 2;
            c.showMenu = false;
            c.className = "table table-striped table-hover ko-grid colorHoverTable " + (d.tableClass ? d.tableClass : "");
            c.ellipsisLength = 16;
            c.searchInitStatus = ko.observable();
            c.searchKey = ko.observable();
            c.searchColumns = d.searchColumns;
            c.primaryColumn = d.primaryColumn;
            c.showPager = !!d.showPager;
            c.sortField = ko.observable(d.defaultSortField);
            c.sortDirection = ko.observable(d.defaultSortDirection);
            c.data = ko.observableArray(d.data);
            c.pageToGo = ko.observable();
            c.sortedData = ko.computed(function() {
                var f = c.sortField();
                var e = c.sortDirection();
                if (f && e) {
                    return c.data().sort(function(h, g) {
                        return e == "ASC" ? h[f].localeCompare(g[f]) : g[f].localeCompare(h[f])
                    })
                } else {
                    return c.data()
                }
            });
            c.checkedCount = ko.observable(0);
            ...
            c.searchText = ko.computed(function() {
                c.clearCheck();
                var e = "";
                if (!c.searchInitStatus()) {
                    e = $.trim(c.searchKey())
                }
                return e
            });
            c.afterSearchData = ko.computed(function() {
                var e = $.trim(c.searchText()).toLowerCase();
                if (e == "") {
                    return c.sortedData()
                } else {
                    return _.filter(c.sortedData(), function(h) {
                        for (var g = 0; g < c.searchColumns.length; g++) {
                            var f = c.searchColumns[g];
                            if (h[f].toLowerCase().indexOf(e) != -1) {
                                return true
                            }
                        }
                        return false
            ...
            c.currentPageIndex = ko.observable(0);
            c.pageSize = d.pageSize || 5;
            c.columns = d.columns || a(ko.utils.unwrapObservable(c.afterSearchData()));
            c.rowClickHandler = d.rowClickHandler;
            c.hasRowClickHandler = !!d.rowClickHandler;
            c.deleteHandler = d.deleteHandler;
            c.idName = d.idName;
            c.tmplType = d.tmplType;
            c.changeTemplateHandler = d.changeTemplateHandler;
            c.maxPageIndex = ko.computed(function() {
                return Math.ceil(ko.utils.unwrapObservable(c.afterSearchData()).length / c.pageSize) - 1
            });
            c.radioClickExtend = d.radioClickHandler;
            c.radioChecked4Column = d.radioChecked4Column;
            c.radioChecked4Value = d.radioChecked4Value;
            c.fixCurrentPageIndex = function(f) {
                if (f < 0) {
                    return 0
                }
                var e = c.maxPageIndex();
                return f > e ? e : f
            };
            c.pagerStart = ko.computed(function() {
                var e = c.fixCurrentPageIndex(c.currentPageIndex());
                var f = c.maxPageIndex();
                var g = 0;
                if (e < c.midNumber || f <= c.allNumber) {
                    g = 1
                } else {
                    if (e + c.sideNumber >= f) {
                        g = f - c.midNumber
            ...
            c.pagerEnd = ko.computed(function() {
                var e = c.fixCurrentPageIndex(c.currentPageIndex());
                var f = c.maxPageIndex();
                var g = 0;
                if (e + c.sideNumber >= f || f <= c.allNumber) {
                    g = f - 1
                } else {
                    if (e < c.midNumber) {
                        g = c.midNumber
            ...
            c.itemsOnCurrentPage = ko.computed(function() {
                c.translate();
                var e = c.fixCurrentPageIndex(c.currentPageIndex());
                var f = c.pageSize * e;
                return c.afterSearchData().slice(f, f + c.pageSize)
            });
            c.changePage = function(e) {
                var e = c.fixCurrentPageIndex(e);
                c.currentPageIndex(e);
                c.clearCheck()
            };
            c.nextPage = function() {
                var e = c.fixCurrentPageIndex(c.currentPageIndex() + 1);
                c.currentPageIndex(e);
                c.clearCheck()
            };
            ...
            c.clearAllChecked = function() {
                $("#pblist-checkall").trigger("click");
                if ($("#pblist-checkall").hasClass("checkbox_selected")) {
                    $("#pblist-checkall").trigger("click")
                }
            };
            c.clickCheckAll = function() {
                if ($("#pblist-checkall").hasClass("checkbox_selected")) {
                    c.checkedCount(0)
                } else {
                    var e = $("#pblist :checkbox").length;
                    c.checkedCount(e)
                }
            };
            c.clickCheck = function(e) {
                var g = $("#chk__" + e + " :checkbox");
                if (g.attr("checked")) {
                ...
                checkCheckbox(g);
                var f = $("#pblist p.checkbox_selected :checkbox").length;
                c.checkedCount(f)
            };
            c.selectedItems = function(g) {
                var f;
                f = $("#pblist :checkbox[checked='checked']");
                var e = [];
                if (g == "primary") {
                    f.each(function() {
                        e.push($(this).attr("primaryValue"))
                    })
                } else {
                    f.each(function() {
                        e.push($(this).val())
            ...
            c.selectedPrimaryValue = function() {
                return c.selectedItems("primary")
            };
            c.radioSelectValue = function() {
            ...
            c.radioSelectedPrimaryValue = function() {
                if ($.browser.msie && event && event.srcElement && event.srcElement.type == "radio") {
                    return event.srcElement.attribute("primaryValue")
                }
                return $("#pblist [name='ko_grid_radio']:checked").attr("primaryValue")
            };
            c.radioClick = function() {
                c.checkedCount(c.checkedCount() + 1)
            };
            c.clearRadioSelect = function() {
                $("#pblist [name='ko_grid_radio']:checked").removeAttr("checked");
                c.checkedCount(0)
            };
            c.setRadioSelect = function(e) {
                $("#pblist [name='ko_grid_radio'][value='" + e + "']").attr("checked", "checked");
                c.checkedCount(1)
            };
            c.sort = function(g) {
                if (g == undefined) {
                ...
                var e = "ASC";
                if (g == f) {
                    e = (c.sortDirection() == "ASC") ? "DESC" : "ASC"
                }
                c.sortField(g);
                c.sortDirection(e);
                c.data(c.data().sort(function(i, h) {
                    return e == "ASC" ? i[g].localeCompare(h[g]) : h[g].localeCompare(i[g])
                }));
                ...
                if (e == "ASC") {
                    $("#ko_grid_th_" + g).removeClass().addClass("ko-grid-th-asc")
                } else {
                    $("#ko_grid_th_" + g).removeClass().addClass("ko-grid-th-desc")
                }
            };
            c.ellipsisText = function(f) {
                if (!f) {
                    return ""
                }
                if ((f.length * 2) < c.ellipsisLength) {
                    return f
                }
                var e = 0;
                for (var g = 0; g < f.length; g++) {
                    if (f.charCodeAt(g) > 256) {
                        e += 2
                    } else {
                        e++
                    }
                    if (e >= c.ellipsisLength) {
                        return f.substring(0, g) + "..."
                    }
                }
                return f
            }
    ...
    var b = new ko.nativeTemplateEngine();
    b.addTemplate = function(c, d) {
        if ($("#" + c)[0]) {
            $("#" + c).remove()
        }
        $("#container").append("<script type='text/html' id='" + c + "'>" + d + "<\/script>")
    };
    ko.bindingHandlers.simpleGrid = {
        init: function(d, e) {
            var c = e();
            if (c.rowClickHandler == undefined) {
                c.rowClickHandler = function() {}
            }
            if (c.deleteHandler == undefined) {
                c.deleteHandler = function() {}
            }
            if (c.radioClickExtend == undefined) {
                c.radioClickHandler = function() {
                    c.radioClick();
                    return true
                }
            } else {
            ...
            ko.utils.arrayMap(c.columns, function(f) {
